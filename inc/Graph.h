...
struct Edge {
    ...
    Edge(Vertex *from, Vertex *to, int weight, int otherWeight) {
        this->from = from;
        this->to = to;
        this->weight = weight;
...
struct Waypoint {
    ...
    void expand() {
        for (int i = 0; i < vertex->edgeList.size(); i++) {
            ...
            temp->otherWeight = vertex->edgeList[i]->otherWeight;
...
struct Graph {
    ...
    void addEdge(Vertex *x, Vertex *y, int w, int o) {
        x->edgeList.append(new Edge(x, y, w, o));
        y->edgeList.append(new Edge(y, x, w, o));
    }

    void addDirectedEdge(Vertex *x, Vertex *y, int w, int o) {
        x->edgeList.append(new Edge(x, y, w, o));
    }
